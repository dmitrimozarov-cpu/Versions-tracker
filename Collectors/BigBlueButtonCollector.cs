...
public class BigBlueButtonCollector(IHttpClientFactory httpClientFactory) : IVersionCollector
{
    public string ProductName => "BigBlueButton";
    ...
    public async Task<IReadOnlyCollection<CollectedVersion>> CollectAsync()
    {
        ...
        var url = "https://api.github.com/repos/bigbluebutton/bigbluebutton/releases?per_page=50";
        ...
        foreach (var release in doc.RootElement.EnumerateArray())
        {
            ...
            // Skip release candidates and beta versions
            if (tag.Contains("-rc", StringComparison.OrdinalIgnoreCase)) continue;
            if (tag.Contains("-beta", StringComparison.OrdinalIgnoreCase)) continue;
            if (tag.Contains("-alpha", StringComparison.OrdinalIgnoreCase)) continue;
