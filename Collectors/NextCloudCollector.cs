...
public class NextCloudCollector(IHttpClientFactory httpClientFactory) : IVersionCollector
{
    public string ProductName => "Nextcloud";

    public async Task<IReadOnlyCollection<CollectedVersion>> CollectAsync()
    {
        var client = httpClientFactory.CreateClient();
        // GitHub API требует указания User-Agent, иначе может возвращать ошибку 403 Forbidden
        client.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("VersionTracker", "1.0"));

        var url = "https://api.github.com/repos/nextcloud/server/releases?per_page=50";
        ...
        {
            // Пропускаем релизы, которые являются предварительными (prerelease) или черновиками (draft)
            ...
            // Нормализуем версию, убирая возможный префикс 'v' (например, "v23.0.0" -> "23.0.0")
